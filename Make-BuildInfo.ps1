...
param(
    ...
    [string]$Out = "$HOME\Desktop\aqw-final\build-info.txt"
)
...
$prod    = (git merge-base HEAD origin/fix/aqw-render-final-performance 2>$null)
if (-not $prod) { $prod = (git merge-base HEAD fork/fix/aqw-render-final-performance 2>$null) }
if (-not $prod) { $prod = (git merge-base HEAD fix/aqw-render-final-performance 2>$null) }
if (-not $prod) { $prod = "unknown - production branch not fetched" }
$prodSubject = if ($prod -match '^[0-9a-f]{40}$') { git log -1 --pretty=%s $prod } else { "" }

@"
=== AQW FINAL DIAGNOSTIC BUILD (phase 2) ===
