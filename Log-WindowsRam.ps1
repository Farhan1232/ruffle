...
param(
    [string]$Out = "$HOME\Desktop\aqw-final\windows-ram.csv",
    [int]$IntervalSeconds = 5
)

New-Item -ItemType Directory -Force -Path (Split-Path $Out) | Out-Null
"timestamp,working_set_mb,working_set_gb,private_bytes_mb,peak_working_set_mb,peak_private_bytes_mb,cpu_seconds" | Set-Content -Encoding UTF8 $Out
...
while ($true) {
    ...
    if ($p) {
        ...
        $pws = [math]::Round($p.PeakWorkingSet64 / 1MB, 1)
        $ppb = [math]::Round($p.PeakPagedMemorySize64 / 1MB, 1)
        "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$ws,$gb,$pb,$pws,$ppb,$cpu" | Add-Content -Encoding UTF8 $Out
